...
// Copyright (c) 2015 The IBTCoin developers
...
unsigned int nTargetSpacing = 1 * 30; // 1 minute
...
const string strMessageMagic = "IBTCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // IBTCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // IBTCoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    {
      nSubsidy = 2200000000 * COIN;
      return nSubsidy + nFees;
    } 

    else if (pindexBest->nHeight < 4204801) {
        nSubsidy = 1 * COIN;
        return nSubsidy + nFees;
    } else if (pindexBest->nHeight < 8409602) {
        nSubsidy = 0.5 * COIN;
        return nSubsidy + nFees;
    } else {
        nSubsidy = 0.25 * COIN;
...
// IBTCoin: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // IBTCoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // IBTCoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // IBTCoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // IBTCoin: track money supply and mint amount info
...
// IBTCoin: total coin age spent in transaction, in the unit of coin-days.
...
// IBTCoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // IBTCoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // IBTCoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // IBTCoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // IBTCoin: compute stake modifier
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (IsProofOfStake())
    {
        ...
        // IBTCoin: check proof-of-stake block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // IBTCoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // IBTCoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // IBTCoin: getblocks may not obtain the ancestor block rejected
    ...
    // IBTCoin: if responsible for sync-checkpoint send it
...
// IBTCoin: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "IBTCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "IBTCoin";
        CTransaction txNew;
        txNew.nTime = 1523003858;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1523003858; //Friday, 24-Mar-17 02:44:04 UTC
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2715600;
        if(fTestNet)
        {
            block.nNonce   = 2715600;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x63d32c7a2dcc6cd7f89c968bb780347817245d532d149b23b041b3c69558c89c"));
        block.print();
        ...
        // IBTCoin: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // IBTCoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // IBTCoin: tell downloading node about the latest block if it's
