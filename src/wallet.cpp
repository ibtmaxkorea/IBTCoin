...
// IBTCoin: total coins staked (non-spendable until maturity)
...
// IBTCoin: get current stake weight
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
		...
		//IBTCoin now iterates each utxo inside of CheckStakeKernelHash()
...
// IBTCoin: check 'spent' consistency between wallet and txindex
// IBTCoin: fix wallet spent state according to txindex
...
// IBTCoin: disable transaction (only for coinstake)
